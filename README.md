...
skills I use every day — feature discovery, plan building, PR review (my own and
others'), Linear ticket authoring, and Prisma migration safety — alongside the
config, plugins, and MCP servers that make them work.
...
```
/plugin marketplace add <path-to-this-repo>      # or a repo URL, e.g. https://github.com/aangieespinoza/claude-code-setup
...
3. Install this plugin (Option A) **or** copy `skills/*` into `~/.claude/skills/`.
4. Install the plugins/marketplaces in [`docs/plugins.md`](./docs/plugins.md) via `/plugin`.
5. Enable the MCP servers in [`docs/mcp-servers.md`](./docs/mcp-servers.md).

**The environment it assumes:** Opus 4.8 with the 1M context window (`opus[1m]`),
proactive output style, medium effort, and an anti-sycophancy mentor persona. See
...
```
ticket → feature-discovery → plan-builder → implement → my-pr-review → agent-pr-creator → push
```

`feature-discovery` grounds the ticket in the actual codebase; `plan-builder`
writes the plan and **stops for approval**; `my-pr-review` self-reviews before
anyone else sees it. Nothing commits, pushes, or opens a PR without my say-so.
...
post**. Nothing reaches GitHub until I approve the list.
